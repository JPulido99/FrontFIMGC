...
const TramitarBorradoraTesis = () => {
  ...
  const handleBuscarAsesor = () => {
    axios
      .get(`http://localhost:8080/users/buscarDocentes?firstName=${nombreAsesor}`)
      .then((response) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/api/expediente/${user.id}/expedienteUserEscuelaBorrador?asesorId=${asesorId}&miembroId=${miembroId}&presidenteId=${presidenteId}&escuelaId=${selectedEscuelaId}&telefono=${telefono}`,
        expedienteData,
        {
          headers: {
