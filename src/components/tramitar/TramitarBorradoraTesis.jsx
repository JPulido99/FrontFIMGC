...
const TramitarBorradoraTesis = () => {
  ...
  const handleBuscarMiembro = () => {
    axios
      .get(`https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/users/buscarDocentes?firstName=${nombreMiembro}`)
      .then((response) => {
        if (response.data && response.data.length > 0) {
          setDocentes(response.data);
  ...
  const handleBuscarPresidente = () => {
    axios
      .get(`https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/users/buscarDocentes?firstName=${nombrePresidente}`)
      .then((response) => {
        if (response.data && response.data.length > 0) {
          setDocentes(response.data);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/api/expediente/${user.id}/expedienteUserEscuelaBorrador?asesorId=${asesorId}&miembroId=${miembroId}&presidenteId=${presidenteId}&escuelaId=${selectedEscuelaId}&telefono=${telefono}`,
        expedienteData,
        {
          headers: {
