...
const ConstTurnitin = () => {
  ...
  const verificarExp = async () => {
    ...
    try {
      ...
      if (veriResponse.data) {
        ...
        const emailiResponse = await axios.get(`http://localhost:8080/api/expediente/emaili/${inputNroDoc}`);
  ...
  const enviarCorreo = async () => {
    ...
    try {
      ...
      const documentoResponse = await axios.post(`http://localhost:8080/api/documento/${nroDoc}/actualizarConstancia`, dataToSendDoc);
