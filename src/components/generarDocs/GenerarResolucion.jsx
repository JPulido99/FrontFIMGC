...
const GenerarResolucion = () => {
  ...
  const verificarExp = async () => {
    ...
    try {
      ...
      const emailiResponse = await axios.get(`http://localhost:8080/api/expediente/todoi/${inputNroDoc}`);
  ...
  const handleBuscarClick = () => {
    axios
      .get(`http://localhost:8080/users/buscarDocentes?firstName=${nombreSecredocente}`)
      .then((response) => {
  ...
  const handleGenerar = () => {
    ...
    if (tramiteExpediente === 'Borrador de Tesis') {
      ...
      axios
        .post(`http://localhost:8080/api/documento/${nroDoc}/actualizarSustentacion?secredocenteId=${secredocenteId}`, documento)
        .then((response) => {
          ...
          const url = `http://localhost:8080/reporte/RDBorrador/download?NroExp=${nroDoc}&tipo=PDF`;
      ...
      axios
        .post(`http://localhost:8080/api/documento/${nroDoc}/actualizarSesionCF?fechaSesionCF=${fechaSesionCFDate}`)
        .then((response) => {
          // Procesar la respuesta del backend si es necesario
          console.log('Expediente creado exitosamente:', response.data);

          //Endpoint de generaacion
          const url = `http://localhost:8080/reporte/RCFBachiller/download?NroExp=${nroDoc}&tipo=PDF`;
          window.open(url, '_blank');
        })
        .catch((error) => {
          // Manejar errores si los hay
      ...
      axios
        ...
        .then((response) => {
          ...
          //Endpoint de generaacion
          const url = `http://localhost:8080/reporte/RCFTitulo/download?NroExp=${nroDoc}&tipo=PDF`;
          window.open(url, '_blank');
        })
        .catch((error) => {
          // Manejar errores si los hay
      ...
      const url = `http://localhost:8080/reporte/RDPlan/download?NroExp=${nroDoc}&tipo=PDF`;
      window.open(url, '_blank');
    }
