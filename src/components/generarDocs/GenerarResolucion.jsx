...
const GenerarResolucion = () => {
  ...
  const verificarExp = async () => {
    ...
    try {
      ...
      const emailiResponse = await axios.get(`https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/api/expediente/todoi/${inputNroDoc}`);
  ...
  const handleBuscarClick = () => {
    axios
      .get(`https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/users/buscarDocentes?firstName=${nombreSecredocente}`)
      .then((response) => {
  ...
  const handleGenerar = () => {
    ...
    if (tramiteExpediente === 'Borrador de Tesis') {
      ...
      axios
        .post(`https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/api/documento/${nroDoc}/actualizarSustentacion?secredocenteId=${secredocenteId}`, documento)
        .then((response) => {
          ...
          const url = `https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/reporte/RDBorrador/download?NroExp=${nroDoc}&tipo=PDF`;
      ...
      axios
        .post(`https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/api/documento/${nroDoc}/actualizarSesionCF?fechaSesionCF=${fechaSesionCFDate}`)
        .then((response) => {
          // Procesar la respuesta del backend si es necesario
          console.log('Expediente creado exitosamente:', response.data);

          //Endpoint de generaacion
          const url = `https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/reporte/RCFBachiller/download?NroExp=${nroDoc}&tipo=PDF`;
          window.open(url, '_blank');
        })
        .catch((error) => {
          // Manejar errores si los hay
      ...
      axios
        ...
        .then((response) => {
          ...
          //Endpoint de generaacion
          const url = `https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/reporte/RCFTitulo/download?NroExp=${nroDoc}&tipo=PDF`;
          window.open(url, '_blank');
        })
        .catch((error) => {
          // Manejar errores si los hay
      ...
      const url = `https://8080-cs-106689005237-default.cs-us-east1-pkhd.cloudshell.dev/reporte/RDPlan/download?NroExp=${nroDoc}&tipo=PDF`;
      window.open(url, '_blank');
    }
